...
def is_hf_model(model_name: str) -> bool:
    # models starting with "hf/" are Hugging Face
...
def get_model(model_id: str, **model_params) -> BaseLLM:
    try:
        if is_openai_model(model_id):
            module = import_module("persona.models.openai_models")
            print("Imported model module:", module.__name__)
            return module.LLM(**model_params)
        elif is_hf_model(model_id):
            module = import_module("persona.models.hf_models")
            print("Imported Hugging Face model module:", module.__name__)
            return module.LLM(model_id=model_id, **model_params)
        else:
...
def get_api_key(model_name: str) -> str:
    ...
    elif is_hf_model(model_name):
        return ""  # No API key needed for local HF models
    else:
