...
def extract_text_from_output(output_text):
    # This is a simple extractor; adapt if your output has more structure
...
if __name__ == "__main__":
    ...
    ## New argument: force local HF model loading
    parser.add_argument("--use_local_model", help="Use local HuggingFace model instead of API", action="store_true")
    ...
    if args.use_local_model:
        model, tokenizer = load_local_hf_model(model_name)
        print(f"Done initializing local HF model: {model_name}")
        # Create dummy model wrapper to unify interface for rest of the script
        class LocalModelWrapper:
            ...
            def generate(self, user_prompt, system_prompt, stop_condition=None, idx=None):
                response = generate_local_hf_response(self.model, self.tokenizer, system_prompt, user_prompt, model_params)
                return response

            def add_usage(self, raw_response):
                # Rough token count: tokens in output only
        ...
        model = LocalModelWrapper(model, tokenizer)
    else:
        # API model loading (existing)
        if args.api_key:
    ...
    if args.experiment_prefix:
        ...
        raw_output_path = f"{output_dir}/{args.experiment_prefix}_{persona_fname}_raw_responses_s{start_idx}_e{end_idx}_temp{model_params.get('temperature', 'NA')}_gen{model_params.get('num_samples', 'NA')}_{dt_string}.jsonl"
        text_output_path = f"{output_dir}/{args.experiment_prefix}_{persona_fname}_text_predictions_s{start_idx}_e{end_idx}_temp{model_params.get('temperature', 'NA')}_gen{model_params.get('num_samples', 'NA')}_{dt_string}.jsonl"
    else:
        persona_fname = persona.replace(" ", "_")
        raw_output_path = f"{output_dir}/{persona_fname}_raw_responses_s{start_idx}_e{end_idx}_temp{model_params.get('temperature', 'NA')}_gen{model_params.get('num_samples', 'NA')}_{dt_string}.jsonl"
        text_output_path = f"{output_dir}/{persona_fname}_text_predictions_s{start_idx}_e{end_idx}_temp{model_params.get('temperature', 'NA')}_gen{model_params.get('num_samples', 'NA')}_{dt_string}.jsonl"
    ...
    with open(raw_output_path, 'w', encoding='utf-8') as raw_writer, \
        ...
        for i, instance in enumerate(test_set):
            ...
            raw_response = model.generate(user_prompt=user_prompt, system_prompt=system_prompt,
                                          stop_condition=dataset.stop_condition, idx=question_id)  # type: ignore
    ...
    print(f"Total time taken: {seconds_taken // 3600}:{(seconds_taken % 3600) // 60}:{seconds_taken % 60} seconds.",
          flush=True)
